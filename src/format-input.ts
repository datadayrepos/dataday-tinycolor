...
import type { HSL, HSLA, HSV, HSVA, RGB, RGBA } from './interfaces'
...
export function stringInputToObject(color: string): any {
  ...
  if (names[color]) {
    ...
    return { a: 0, b: 0, format: 'name', g: 0, r: 0 }
  }
  ...
  if (match)
    return { b: match[3], g: match[2], r: match[1] }

  match = matchers.rgba.exec(color)
  if (match)
    return { a: match[4], b: match[3], g: match[2], r: match[1] }

  match = matchers.hsl.exec(color)
  if (match)
    return { h: match[1], l: match[3], s: match[2] }

  match = matchers.hsla.exec(color)
  if (match)
    return { a: match[4], h: match[1], l: match[3], s: match[2] }
  ...
  if (match)
    return { a: match[4], h: match[1], s: match[2], v: match[3] }
